...
def ReadDicomfiles(abspath_PhaseID):
    ...
    FileNms_slices_sorted_stack:  (dict)    List of filenames sorted by slice location
    """
    slices = []
    FileNms_slices =  []
    
    listSeries_files = sorted(list(processDicoms.get_only_filesindirectory(abspath_PhaseID)))
    ...
    for n in range(len_listSeries_files):
        ...
        absp_fsID = abspath_PhaseID+os.sep+listSeries_files[n]
                
        if(absp_fsID!=abspath_PhaseID+os.sep+'DIRCONTENTS.txt'):
    ...
    print '''\nprint most left SLICE LOCATION: ''' 
    print str(FileNms_slices_sorted_stack[0,1]), str(FileNms_slices_sorted_stack[0,0])
            
    return len_listSeries_files, FileNms_slices_sorted_stack
